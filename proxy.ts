...
export async function proxy(request: NextRequest) {
  ...
  if (ticketMatch) {
    ...
    return NextResponse.rewrite(new URL(`/api/seo/redirect-ticket?id=${ticketId}`, request.url));
  }
  ...
  const isBingbot =
    userAgent.toLowerCase().includes("bingbot") || userAgent.toLowerCase().includes("bingpreview");
  if (isBingbot) {
    response.headers.set("Cache-Control", "public, max-age=0, must-revalidate");
    response.headers.set("Vary", "User-Agent");
  } else {
    // 3.7 ⚡ PERFORMANCE: Cache-Control Header Optimization
    const isStaticPage = ["/terms", "/privacy", "/how-it-works", "/support", "/cookies"].some(
