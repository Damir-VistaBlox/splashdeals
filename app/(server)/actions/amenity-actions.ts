...
export async function updateFacilityAmenitiesAction(
  ...
  try {
    const validation = await validateAction(updateFacilityAmenitiesSchema, {
      facilityId,
      amenities: data,
      lastUpdatedAt,
    });
    ...
    if (lastUpdatedAt) {
      ...
      if (lastAmenityUpdate && lastAmenityUpdate !== lastUpdatedAt) {
        throw new Error(
          "CONFLICT: Amenities were modified by another admin while you were editing. Please refresh and try again.",
        );
      }
    }
          ...
          if (!existing) {
            ...
            if (existing.scheduledAt?.toISOString() !== item.scheduledAt) {
              auditEntries.push({
                ...
                newValue: item.scheduledAt,
              });
            }
            if (existing.isFeatured !== item.isFeatured) {
              auditEntries.push({
                action: "PROMOTION",
                oldValue: existing.isFeatured ? "FEATURED" : "STANDARD",
                newValue: item.isFeatured ? "FEATURED" : "STANDARD",
              });
            }
          }
