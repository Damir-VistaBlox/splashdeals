...
export function HomeBlogStrip({ dict, posts }: { dict: HomeDict; posts: Post[] }) {
  return (
    <section className="mx-auto hidden max-w-7xl px-4 py-10 sm:px-6 sm:py-14 md:block md:px-12">
            ...
            <Link key={post.id} href={`/blog/${post.slug}`} className="group">
              <Card className="hover:border-primary/40 overflow-hidden transition-colors duration-150">
                ...
                <div className="space-y-2 p-4">
                  <h3 className="group-hover:text-primary line-clamp-2 text-base font-black tracking-tight transition-colors md:text-sm">
                    {post.title}
                  </h3>
                  {post.excerpt ? (
                    <p className="text-muted-foreground line-clamp-2 text-sm leading-relaxed md:text-xs">
                      {post.excerpt}
                    </p>
                  ) : null}
                </div>
