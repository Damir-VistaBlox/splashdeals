    ...
    <Link href={`/facilities/${facility.category.toLowerCase()}/${facility.slug}`} className="block">
      ...
      >
        ...
        <div className="relative z-10 p-6 flex flex-col gap-1 w-full bg-gradient-to-t from-slate-950/80 via-slate-950/40 to-transparent">
          <span className="text-[10px] font-black text-cyan-400 uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
            <Icon name="navigation" className="text-[12px] text-cyan-400 rotate-45" />
            {dict?.categories?.[facility.category.toLowerCase()] || facility.category}
          </span>
          <h3 className="text-2xl font-black text-white italic tracking-tighter uppercase mb-3 leading-none group-hover:text-cyan-400 transition-colors">
            {facility.name}
          </h3>
          
          <div className="flex flex-col gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">
            <div className="flex items-center gap-2">
              <Icon name="location_on" className="text-[14px] text-slate-500 shrink-0" />
            ...
            <div className="flex items-center justify-between border-t border-white/5 pt-4 mt-2">
              {facility.tickets[0] ? (
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  {fromLabel} <span className="text-base text-cyan-400 font-black block leading-none mt-1">{Number(facility.tickets[0].price)} {facility.tickets[0].currency}</span>
                </div>
              ) : (
                <div />
              )}
              
              <div className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[0.15em] text-cyan-400 bg-cyan-400/10 border border-cyan-400/20 px-4 py-2.5 rounded-xl transition-all duration-300 group-hover:bg-cyan-400 group-hover:text-slate-950 group-hover:border-cyan-400 group-hover:translate-x-1 shadow-lg shadow-cyan-950/10">
