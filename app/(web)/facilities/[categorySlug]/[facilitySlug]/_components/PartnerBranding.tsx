...
export function PartnerBranding({ logoUrl, name }: PartnerBrandingProps) {
  ...
  return (
     <div className="glass-frost p-6 rounded-[2rem] border border-white/5 bg-white/[0.02] flex items-center gap-5 transition-all hover:border-cyan-500/20 hover:bg-white/[0.04] duration-500 group">
        <div className="relative h-24 w-24 rounded-3xl overflow-hidden border border-white/10 bg-slate-950/50 p-2.5 backdrop-blur-md flex items-center justify-center flex-shrink-0 shadow-2xl group-hover:scale-105 transition-transform duration-500">
          <Image 
             src={logoUrl} 
             alt={`${name} Logo`}
             fill
