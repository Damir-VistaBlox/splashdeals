    ...
    <Card className="p-6 border-rose-500/20 bg-rose-950/5 backdrop-blur-md space-y-6">
      <div className="flex items-center justify-between pb-4 border-b border-rose-500/10">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-xs font-black uppercase tracking-wider text-foreground">Danger Zone</h3>
      ...
      <div className="space-y-4">
        {!isSuperAdmin ? (
          <div className="flex items-start gap-3 p-4 rounded-xl bg-background/40 border border-border/50">
            <Icon name="lock" className="text-[16px] text-muted-foreground mt-0.5 shrink-0" />
            <div className="space-y-1">
              <p className="text-xs font-black uppercase tracking-wider text-foreground/80">Administrative Guard Active</p>
          ...
          <div className="flex items-start gap-3 p-4 rounded-xl bg-background/40 border border-rose-500/10">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 rounded-xl bg-background/40 border border-rose-500/10">
            <div className="space-y-1 max-w-xl">
              <p className="text-xs font-black uppercase tracking-wider text-foreground">Purge Facility Node</p>
              <p className="text-[10px] text-muted-foreground font-medium leading-normal">
                Permanently delete <strong className="text-foreground/80">{facilityName}</strong> along with all associated tickets, operating hours, amenities, closures, and staff assignments. This action is absolute and cannot be undone.
              </p>
            </div>
              ...
              <DialogContent className="bg-background border-border text-foreground max-w-md rounded-2xl p-6 outline-none">
                <DialogHeader className="space-y-3">
                  ...
                  <DialogDescription className="text-xs text-muted-foreground leading-normal">
                    This action is <strong className="text-rose-400 uppercase">destructive</strong> and will completely wipe <strong className="text-foreground">{facilityName}</strong> from the database. It will immediately cascade to delete:
                  </DialogDescription>
                </DialogHeader>

                <div className="py-2 space-y-2">
                  <ul className="text-[9px] font-mono text-muted-foreground list-disc pl-5 space-y-1 uppercase tracking-wider">
                  ...
                  <div className="space-y-2 pt-4 border-t border-border/50">
                    <label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">
                      To confirm, type the exact facility name <span className="text-foreground select-none">&quot;{facilityName}&quot;</span> below:
                    </label>
                    <Input
                      value={confirmName}
                      onChange={(e) => setConfirmName(e.target.value)}
                      placeholder={facilityName}
                      className="bg-muted/30 border-border text-foreground text-xs h-10 px-3 rounded-lg focus:border-rose-500/50"
                      disabled={isPending}
                    />
                  </div>
                </div>
                ...
                <DialogFooter className="flex flex-col sm:flex-row gap-2 mt-4">
                  ...
                  <Button
                    ...
                    className="h-10 bg-rose-600 hover:bg-rose-500 disabled:bg-rose-950/20 disabled:text-rose-900/50 text-foreground text-[9px] font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center min-w-[120px]"
                  >
                    {isPending ? (
