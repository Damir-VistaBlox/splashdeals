...
export function Breadcrumbs() {
  ...
  const breadcrumbSegments =
    segments.length > 0 && segments[0] === "admin" ? segments.slice(1) : segments;

  if (breadcrumbSegments.length === 0) return null;

  return (
    <nav className="text-muted-foreground/60 flex items-center gap-1.5 text-[10px] font-black tracking-widest uppercase">
      <Link
        href="/admin"
        className="hover:text-primary group flex items-center gap-1 transition-colors"
      >
        <Icon name="home" className="text-[12px] transition-transform group-hover:scale-110" />
      </Link>
